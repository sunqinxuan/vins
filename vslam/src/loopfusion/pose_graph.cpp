...
PoseGraph::PoseGraph(std::string config_file_path) {
  ...
  INFO("[LoopFusion][PoseGraph] config_file: ", config_file);
  cv::FileStorage config(config_file, cv::FileStorage::READ);
  if (!config.isOpened()) {
    ERROR("[LoopFusion][PoseGraph] ERROR: wrong path to config file!");
  }
  std::string out_path;
  ...
  vins_result_path_ = config_file_path + out_path + "vio_loop.csv";
  std::cout << "[LoopFusion][PoseGraph] vins result path: " << vins_result_path_
            << std::endl;
  std::ofstream fout(vins_result_path_, std::ios::out);
  fout.close();

  pose_graph_save_path_ = config_file_path + out_path + "pose_graph/";
  std::cout << "[LoopFusion][PoseGraph] pose graph save path: "
            << pose_graph_save_path_ << std::endl;

  std::string vocabulary_file = config_file_path + "brief_k10L6.bin";
  INFO("[LoopFusion][PoseGraph] vocabulary_file: ", vocabulary_file);
...
void PoseGraph::registerPub(ros::NodeHandle &n) {
  pub_pg_path =
      n.advertise<nav_msgs::Path>("/loopfusion/pose_graph_path", 1000);
  pub_base_path = n.advertise<nav_msgs::Path>("/loopfusion/base_path", 1000);
  pub_pose_graph = n.advertise<visualization_msgs::MarkerArray>(
      "/loopfusion/pose_graph", 1000);
  for (int i = 1; i < 10; i++)
    pub_path[i] =
        n.advertise<nav_msgs::Path>("/loopfusion/path_" + to_string(i), 1000);
}
...
void PoseGraph::addKeyFrame(KeyFrame *cur_kf, bool flag_detect_loop) {
  ...
  if (sequence_cnt != cur_kf->sequence) {
	...
	cout<<"w_r_vio = "<<endl<<w_r_vio<<endl;
  cur_kf->getVioPose(vio_P_cur, vio_R_cur);
	cout<<"vio_P_cur = "<<vio_P_cur.transpose()<<endl;
	cout<<"vio_R_cur = "<<endl<<vio_R_cur<<endl;
  vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
  vio_R_cur = w_r_vio * vio_R_cur;
	cout<<"trans vio_P_cur = "<<vio_P_cur.transpose()<<endl;
	cout<<"trans vio_R_cur = "<<endl<<vio_R_cur<<endl;
  ...
  if (loop_index != -1) {
    ...
    if (cur_kf->findConnection(old_kf)) {
      ...
      if (use_imu) {
        shift_yaw =
            Converter::R2ypr(w_R_cur).x() - Converter::R2ypr(vio_R_cur).x();
        shift_r = Converter::ypr2R(Vector3d(shift_yaw, 0, 0));
      } else
  ...
  if (SAVE_LOOP_PATH) {
    ofstream loop_path_file(vins_result_path_, ios::app);
...
int PoseGraph::detectLoop(KeyFrame *keyframe, int frame_index) {
  ...
  if (ret.size() >= 1 && ret[0].Score > 0.05)
    for (unsigned int i = 1; i < ret.size(); i++) {
      ...
      //if (debug_image_)
      //{
      //    cv::imshow("loop_result_sqx", loop_result);
      //    cv::waitKey(20);
      //}
...
void PoseGraph::optimize4DoF() {
  while (true) {
    ...
    if (cur_index != -1) {
      ...
      ofstream fp("/home/sun/vins_sqx/loop.txt", std::ios::app);
      ...
      for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
        if ((*it)->index < first_looped_index)
          continue;
        (*it)->local_index = i;
        fp << "index = " << (*it)->index << endl;
        fp << "local_index = " << (*it)->local_index << endl;
        Quaterniond tmp_q;
        Matrix3d tmp_r;
        Vector3d tmp_t;
        ...
        t_array[i][2] = tmp_t(2);
        q_array[i] = tmp_q;
        fp << "VIO-trans = " << tmp_t.transpose() << endl;
        fp << "VIO-quat = " << tmp_q.w() << " " << tmp_q.x() << " " << tmp_q.y()
           << " " << tmp_q.z() << endl;

        Vector3d euler_angle = Converter::R2ypr(tmp_q.toRotationMatrix());
        euler_array[i][0] = euler_angle.x();
        euler_array[i][1] = euler_angle.y();
        euler_array[i][2] = euler_angle.z();

        sequence_array[i] = (*it)->sequence;
        fp << "sequence = " << (*it)->sequence << endl;
        ...
        for (int j = 1; j < 5; j++) {
          if (i - j >= 0 && sequence_array[i] == sequence_array[i - j]) {
            Vector3d euler_conncected =
                Converter::R2ypr(q_array[i - j].toRotationMatrix());
        ...
        if ((*it)->has_loop) {
          ...
          Vector3d euler_conncected =
              Converter::R2ypr(q_array[connected_index].toRotationMatrix());
          Vector3d relative_t;
          relative_t = (*it)->getLoopRelativeT();
          double relative_yaw = (*it)->getLoopRelativeYaw();
          fp << "has loop: " << endl;
          fp << "euler_connected = " << euler_conncected.transpose() << endl;
          fp << "relative_t = " << relative_t.transpose() << endl;
          fp << "relative_yaw = " << relative_yaw << endl;
      ...
      fp << endl << "ceres summary -----------------------------------" << endl;
      fp << summary.FullReport() << "\n";
      ...
      m_drift.lock();
      yaw_drift = Converter::R2ypr(cur_r).x() - Converter::R2ypr(vio_r).x();
      r_drift = Converter::ypr2R(Vector3d(yaw_drift, 0, 0));
      t_drift = cur_t - r_drift * vio_t;
      m_drift.unlock();
      fp << "t_drift " << t_drift.transpose() << endl;
      fp << "r_drift " << Converter::R2ypr(r_drift).transpose() << endl;
      fp << "yaw drift " << yaw_drift << endl;
      fp.close();
  ...
  while (true) {
    ...
    if (cur_index != -1) {
      ...
      */
      ...
      // cout << "r_drift " << Converter::R2ypr(r_drift).transpose() << endl;
...
void PoseGraph::updatePath() {
  ...
  if (SAVE_LOOP_PATH) {
    ofstream loop_path_file_tmp(vins_result_path_, ios::out);
...
void PoseGraph::savePoseGraph() {
  ...
  TicToc tmp_t;
  FILE *pFile;
  printf("pose graph path: %s\n", pose_graph_save_path_.c_str());
  printf("pose graph saving... \n");
  string file_path = pose_graph_save_path_ + "pose_graph.txt";
  ...
  for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
    ...
    if (debug_image_) {
      image_path =
          pose_graph_save_path_ + to_string((*it)->index) + "_image.png";
    ...
    brief_path =
        pose_graph_save_path_ + to_string((*it)->index) + "_briefdes.dat";
    std::ofstream brief_file(brief_path, std::ios::binary);
    keypoints_path =
        pose_graph_save_path_ + to_string((*it)->index) + "_keypoints.txt";
    FILE *keypoints_file;
...
void PoseGraph::loadPoseGraph() {
  ...
  while (fscanf(pFile,
    ...
    */
    ...
    if (debug_image_) {
      image_path = pose_graph_save_path_ + to_string(index) + "_image.png";
    ...
    string brief_path =
        pose_graph_save_path_ + to_string(index) + "_briefdes.dat";
    std::ifstream brief_file(brief_path, std::ios::binary);
    string keypoints_path =
        pose_graph_save_path_ + to_string(index) + "_keypoints.txt";
    FILE *keypoints_file;
