...
void KeyFrame::loadConfigFile(std::string config_file_path) {
  std::string config_file = config_file_path + "config.yaml";
  cv::FileStorage config(config_file, cv::FileStorage::READ);
  if (!config.isOpened()) {
    ERROR("[LoopFusion][keyframe] ERROR: wrong path to config file!");
    return;
  }
  ...
  image_rows_ = config["image_height"];
  image_cols_ = config["image_width"];

  config.release();
}

void KeyFrame::computeWindowBRIEFPoint() {
  BriefExtractor extractor(brief_pattern_file_.c_str());
...
void KeyFrame::PnPRANSAC(const vector<cv::Point2f> &matched_2d_old_norm,
  ...
  Matrix3d R_w_c = origin_vio_R * Tic0_calib_.linear();
  Vector3d T_w_c = origin_vio_T + origin_vio_R * Tic0_calib_.translation();
  ...
  PnP_R_old = R_w_c_old * Tic0_calib_.linear().transpose();
  PnP_T_old = T_w_c_old - PnP_R_old * Tic0_calib_.translation();
}
...
*/
  ...
  if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) {
    ...
    if (debug_image_) {
      ...
      cv::Mat notation(50, image_cols_ + gap + image_cols_, CV_8UC3,
      ...
      putText(notation,
              ...
              cv::Point2f(20 + image_cols_ + gap, 30), cv::FONT_HERSHEY_SIMPLEX,
              1, cv::Scalar(255), 3);
