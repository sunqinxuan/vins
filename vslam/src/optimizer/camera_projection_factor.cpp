...
bool ProjectionOneFrameTwoCamFactor::Evaluate(double const *const *parameters,
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jaco_ex.rightCols<3>() =
          ric2.transpose() * ric * -Converter::skewSymmetric(pts_camera_i);
    ...
    if (jacobians[1]) {
      ...
      Eigen::Matrix<double, 3, 6> jaco_ex;
      jaco_ex.leftCols<3>() = -ric2.transpose();
      jaco_ex.rightCols<3>() = Converter::skewSymmetric(pts_camera_j);
...
void ProjectionOneFrameTwoCamFactor::check(double **parameters) {
  ...
  for (int k = 0; k < 14; k++) {
    ...
    if (a == 0)
      ...
      qic = qic * Converter::deltaQ(delta);
    else if (a == 2)
      tic2 += delta;
    else if (a == 3)
      qic2 = qic2 * Converter::deltaQ(delta);
    else if (a == 4) {
      if (b == 0)
        inv_dep_i += delta.x();
      else
...
bool ProjectionTwoFrameOneCamFactor::Evaluate(double const *const *parameters,
                                              double *residuals,
                                              double **jacobians) const {
  // fp.open("/home/sun/vins_sqx/eval.txt", std::ios::app);
  TicToc tic_toc;
  ...
  // fp << "Qi = " << Qi.w() << " " << Qi.x() << " " << Qi.y() << " " << Qi.z()
  //   << std::endl;
  // fp << "Pj = " << Pj.transpose() << std::endl;
  // fp << "Qj = " << Qj.w() << " " << Qj.x() << " " << Qj.y() << " " << Qj.z()
  //   << std::endl;
  // fp << "tic = " << tic.transpose() << std::endl;
  // fp << "qic = " << qic.w() << " " << qic.x() << " " << qic.y() << " "
  //   << qic.z() << std::endl;
  // fp << "inv_dep_i = " << inv_dep_i << std::endl;
  ...
  residual = sqrt_info * residual;
  // fp << "sqrt_info*residual = " << residual.transpose() << std::endl;
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri *
                              -Converter::skewSymmetric(pts_imu_i);

      jacobian_pose_i.leftCols<6>() = reduce * jaco_i;
      jacobian_pose_i.rightCols<1>().setZero();

      // fp << "jacobian_pose_i = " << std::endl << jacobian_pose_i <<
      // std::endl;
    }
    ...
    if (jacobians[1]) {
      ...
      jaco_j.rightCols<3>() =
          ric.transpose() * Converter::skewSymmetric(pts_imu_j);

      jacobian_pose_j.leftCols<6>() = reduce * jaco_j;
      jacobian_pose_j.rightCols<1>().setZero();

      // fp << "jacobian_pose_j = " << std::endl << jacobian_pose_j <<
      // std::endl;
    }
    if (jacobians[2]) {
      Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> jacobian_ex_pose(
      ...
      jaco_ex.rightCols<3>() =
          -tmp_r * Converter::skewSymmetric(pts_camera_i) +
          Converter::skewSymmetric(tmp_r * pts_camera_i) +
          Converter::skewSymmetric(
              ric.transpose() * (Rj.transpose() * (Ri * tic + Pi - Pj) - tic));
      jacobian_ex_pose.leftCols<6>() = reduce * jaco_ex;
      jacobian_ex_pose.rightCols<1>().setZero();

      // fp << "jacobian_ex_pose = " << std::endl << jacobian_ex_pose <<
      // std::endl;
    }
    if (jacobians[3]) {
      ...
      // fp << "jacobian_feature = " << jacobian_feature.transpose() <<
      // std::endl;
    }
    if (jacobians[4]) {
      ...
      jacobian_td = reduce * ric.transpose() * Rj.transpose() * Ri * ric *
                    ...
                    sqrt_info * velocity_j.head(2);
      // fp << "jacobian_td = " << jacobian_td.transpose() << std::endl;
    }
  }
  sum_t += tic_toc.toc();
  // fp.close();
...
void ProjectionTwoFrameOneCamFactor::check(double **parameters) {
  ...
  for (int k = 0; k < 20; k++) {
    ...
    if (a == 0)
      ...
      Qi = Qi * Converter::deltaQ(delta);
    else if (a == 2)
      Pj += delta;
    else if (a == 3)
      Qj = Qj * Converter::deltaQ(delta);
    else if (a == 4)
      tic += delta;
    else if (a == 5)
      qic = qic * Converter::deltaQ(delta);
...
bool ProjectionTwoFrameTwoCamFactor::Evaluate(double const *const *parameters,
  ...
  if (jacobians) {
    ...
    if (jacobians[1]) {
      ...
      jaco_j.rightCols<3>() =
          ric2.transpose() * Converter::skewSymmetric(pts_imu_j);
    ...
    if (jacobians[2]) {
      ...
      jaco_ex.rightCols<3>() = ric2.transpose() * Rj.transpose() * Ri * ric *
                               -Converter::skewSymmetric(pts_camera_i);
      jacobian_ex_pose.leftCols<6>() = reduce * jaco_ex;
      jacobian_ex_pose.rightCols<1>().setZero();
    }
    if (jacobians[3]) {
