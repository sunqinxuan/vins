...
private:
  ...
  void cleanFoundConnectedQuads(std::vector<ChessboardQuadPtr> &quadGroup,
                                cv::Size patternSize);
  ...
  int augmentBestRun(std::vector<ChessboardQuadPtr> &candidateQuads,
                     int candidateDilation,
                     std::vector<ChessboardQuadPtr> &existingQuads,
                     int existingDilation);

  void generateQuads(std::vector<ChessboardQuadPtr> &quads, cv::Mat &image,
                     int flags, int dilation, bool firstRun);
  ...
  void
  getQuadrangleHypotheses(const std::vector<std::vector<cv::Point>> &contours,
